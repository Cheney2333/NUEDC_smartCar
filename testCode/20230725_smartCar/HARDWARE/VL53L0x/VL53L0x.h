...
//我使用8位数字作为地址，LSB必须为0，这样我才能根据读写正确地覆盖最后一位
...
//从寄存器值中解码VCSEL（垂直腔面发射激光器）脉冲周期（以PCLK为单位）
//基于VL53L0X_decode_vcsel_period（）
#define decodeVcselPeriod(reg_val)      (((reg_val) + 1) << 1)

// Encode VCSEL pulse period register value from period in PCLKs
// based on VL53L0X_encode_vcsel_period()

//从PCLK中的周期编码VCSEL脉冲周期寄存器值
//基于VL53L0X_encode_vcsel_period（）
...
//从PCLK中的VCSEL周期计算*纳秒*中的宏周期
//基于VL53L0X_calc_macro_period_ps（）
// PLL_period_ps = 1655; macro_period_vclks = 2304
#define calcMacroPeriod(vcsel_period_pclks) ((((uint32_t)2304 * (vcsel_period_pclks) * 1655) + 500) / 1000)

// register addresses from API vl53l0x_device.h (ordered as listed there)

//来自API vl53l0x_device.h的寄存器地址（按照列出的顺序排列）
enum regAddr {
  SYSRANGE_START                              = 0x00,
...
//将芯片 I2C 和 LIB 配置为“new_addr”（8 位，LSB=0）
...
//初始化并配置传感器。
//如果可选参数 io_2v8 为 1，则传感器配置为 2V8 模式（2.8 V I/O）;
//如果为 0，则传感器保留在 1V8 模式。如果初始化成功完成，则返回 1。
